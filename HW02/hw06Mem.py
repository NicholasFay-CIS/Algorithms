...
def find_items(size, food_array, W, v, s):
	...
	"""
	while(W > 0):
		#while the budget is not zero
		#update the frequency array of food items
		food_array[size[W]] += 1
		#subtract the food item cost from our budget
		W -= v[size[W]]
	for i in range(0, len(food_array)):
		if(food_array[i] != 0):
			#go through the food array and print the number of items per non zero menu item
			print("{} {}".format(s[i], food_array[i]))
	return 

def CMINM(w, v, c, MCAL, size, n):
	"""
	...
	if w == 0:
		return 0
	# if at w it is undefined
	if MCAL[w] == sys.maxint:
		for i in range(0, n):
			#value at index i is less than w
			if v[i] <= w:
				#recursive call 
				temp = CMINM(w - v[i], v, c, MCAL, size, n)
				if(temp + c[i] < MCAL[w]):
					#update the list
					MCAL[w] = temp + c[i]
...
def main():
	#get the input file
	inputfile = sys.stdin
	#changes the max recursion limit so can pass more test cases
	...
	min_cal = CMINM(W, v, c, MCAL, size, n)
