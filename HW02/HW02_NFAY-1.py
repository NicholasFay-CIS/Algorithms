...
def Print_Paths(list__, nodes):
	...
	for edge in list__:
		...
		#if the distance at v is greater than u + 1
		if(short_paths[v-1] > short_paths[u-1] + 1):
			short_paths[v-1] = short_paths[u-1] + 1

		#if the dist of v equals u + 1, then they are one distinct path.
		if(short_paths[v-1] == short_paths[u-1] + 1):
		...
		#if the distance at v is less than than u + 1
		if(long_paths[v-1] < long_paths[u-1] + 1):
			long_paths[v-1] = long_paths[u-1] + 1

		#else we account for if dist of v equals u + 1, then they are one distinct path.
		elif(long_paths[v-1] == long_paths[u-1] + 1):
	...
	print("Number of short paths: {}".format(shortd))
	print("Longest path: {}".format(long_))
	print("Number of long paths: {}".format(longd))
	return 
